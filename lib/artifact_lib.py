...
class ArtifactRegistry(object):
  ...
  def GetArtifactNames(cls, *args, **kwargs):
    return {a.name for a in cls.GetArtifacts(*args, **kwargs)}
  ...
  def SearchDependencies(cls, os_name, artifact_name_list,
    ...
    for artifact in artifact_objs:
      if expansions := artifact.GetArtifactPathDependencies():
        ...
        new_artifact_names = cls.GetArtifactNames(os_name=os_name,
                                                  provides=expansions)
        if missing_artifacts := new_artifact_names - artifact_deps:
  ...
  for match in INTERPOLATED_REGEX.finditer(pattern):
    ...
    try:
      if "." in match.group(1):     # e.g. %%users.username%%
        ...
        if not kb_value:
          ...
          for value in kb_value:
            if sub_attr := value.Get(attr_name):
      ...
      raise KnowledgeBaseInterpolationError(
          f"Failed to interpolate {pattern} with the knowledgebase. {e}")
...
def ArtifactsFromYaml(yaml_content):
  ...
  try:
    ...
    raise ArtifactDefinitionError(f"Invalid json for artifact: {e}")
  ...
  for artifact_dict in raw_list:
    ...
    try:
      ...
      raise ArtifactDefinitionError(
          f'Invalid artifact definition for {artifact_dict.get("name")}: {e}')
    ...
    try:
      files_to_load.extend(
          ...
          if (file_name.endswith(".json") or file_name.endswith(".yaml")
              and not file_name.startswith("test")))
    except (IOError, OSError):
      logging.warn("Error loading artifacts from %s", directory)

  return [] if not files_to_load else LoadArtifactsFromFiles(files_to_load)
